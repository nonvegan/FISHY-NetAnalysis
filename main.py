...
def write_metric_figure(df, zeek_file, metric_aggr, metric, metric_interval):
    fig = go.Figure()
    fig.update_layout(
        title={
            "text": "{} {}({})/{}".format(
    ...
    if not path.exists("./metrics/{}/{}".format(metric_aggr, metric)):
        makedirs("./metrics/{}/{}".format(metric_aggr, metric))

    fig.write_html(
        "metrics/{}/{}/{}_{}_{}_{}.html".format(
            metric_aggr, metric, zeek_file, metric_aggr, metric, metric_interval
        )
    )
...
for metric_interval in conn_metric_intervals:
    ...
    for conn_metric_to_sum in conn_metrics_to_sum:
        ...
        write_metric_figure(conn_df, "conn", "sum", conn_metric_to_sum, metric_interval)
